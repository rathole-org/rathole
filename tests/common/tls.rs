...
use rcgen::{
    ...
    Issuer, KeyPair, KeyUsagePurpose, PKCS_RSA_SHA256,
};
...
// Match the RSA/3DES/SHA-256 identity generated by the documented OpenSSL
// example while avoiding OpenSSL's RC2-based `-legacy` defaults.
...
impl TlsTestConfig {
    pub fn from_template(template_path: impl AsRef<Path>) -> Result<Self> {
        ...
        let ca_key = generate_rsa_key_pair().context("failed to generate test CA key")?;
        ...
        let server_key = generate_rsa_key_pair().context("failed to generate test server key")?;
...
fn generate_rsa_key_pair() -> Result<KeyPair> {
    ...
    KeyPair::from_pkcs8_pem_and_sign_algo(&private_key_pem, &PKCS_RSA_SHA256)
        .context("failed to load generated RSA key")
}
